...
void handle_keys(KEY_EVENT_RECORD kev, enum ControlState state) {
    ...
    // Shared between CS_EDIT and CS_COMMAND
    if (state & (CS_EDIT | CS_COMMAND)) {
        ...
        if (kev.dwControlKeyState & LEFT_CTRL_PRESSED) {
            ...
            switch (kev.wVirtualKeyCode) {
                // wtf
                case VK_LEFT:;
                    win = get_adjacent_win(get_active_window(), DIR_LEFT);
                    if (win) set_active_window(win);
                    break;
                case VK_RIGHT:;
                    win = get_adjacent_win(get_active_window(), DIR_RIGHT);
                    if (win) set_active_window(win);
                    break;
                case VK_UP:;
                    win = get_adjacent_win(get_active_window(), DIR_UP);
                    if (win) set_active_window(win);
                    break;
                case VK_DOWN:;
                    win = get_adjacent_win(get_active_window(), DIR_DOWN);
                    if (win) set_active_window(win);
                    break;
            }
        } else {
